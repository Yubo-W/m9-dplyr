...
gain.or.lose <- mean(flights$time.gained, na.rm = TRUE)

# Create a data.frame that is of flights headed to seatac ('SEA'), 
flights.to.SEA <- filter(flights, dest %in% c('SEA'))

# On average, did flights to seatac gain or loose time?
gain.or.lose.SEA <- mean(flights.to.SEA$time.gained, na.rm = TRUE)
